...
class CollPolicyViewSet(viewsets.ViewSet):
    ...
    def get(self):
        ...
        try:
            ...
            # http://127.0.0.1:1111/v1/api_collection_policy/?sidx=name&sord=asc&name=TEST&ostype=CISCO&cli_command=clock&desc=&search_fields=name,ostype,cli_command,desc,snmp_oid
            query_data = {
                'name': self.name,
                'desc': self.desc,
            ...
            if search_conditions:
                ...
                queryset = CollPolicy.objects.filter(**{'policy_type': self.policy_type})
                if self.id is not '':
